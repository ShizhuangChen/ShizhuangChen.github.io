# Publications markdown generator for AcademicPages (Jekyll GitHub Pages).
#
# Reads a publications table (TSV or CSV) and generates one Markdown file per
# row under the repository's _publications/ -- the Jekyll collection source
# files the site renders. This is the generator side of a source -> generated
# workflow: edit the table, re-run this script, then commit the regenerated
# Markdown. Do NOT hand-edit the generated Markdown files; they will be
# overwritten on the next run.
#
# Usage:
#   python3 scripts/publications.py data/<table>.tsv
#   (run from the markdown_generator/ folder; the input path is relative to
#   your current directory)
#
# Input -- the first line must be the header, in either layout:
#   legacy : pub_date, title, venue, excerpt, citation, url_slug, paper_url, slides_url
#   updated: the same headers plus a trailing `category` column
#
# Requirements:
#   - `pub_date` must be formatted as YYYY-MM-DD.
#   - `url_slug` becomes the file base name and the permalink tail, so the
#     file is YYYY-MM-DD-<url_slug>.md and the permalink is
#     https://<domain>/publication/YYYY-MM-DD-<url_slug>.
#   - `category` (books/manuscripts/conferences) is emitted only for the
#     updated header; legacy rows default to "manuscripts".
#   - `excerpt`, `paper_url`, `slides_url` may be blank; the rest are required.
#   - `paper_url` / `slides_url` become the `paperurl` / `slidesurl` front
#     matter fields plus "Download paper/slides here" links on the page.
#
# Output: <repo-root>/_publications/YYYY-MM-DD-<url_slug>.md  (one per row)
import csv
import os
import sys

# Project root = the directory that contains the repository's top level.
# The script lives in markdown_generator/scripts/, so three levels up lands on
# the repo root. Output paths are resolved against it so the script works no
# matter which directory it is invoked from.
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
PUBLICATIONS_DIR = os.path.join(PROJECT_ROOT, "_publications")
...
def create_md(lines: list, layout: list):
    for item in lines:
        ...
        if len(str(item[layout.index('slides_url')])) > 5:
        ...
        if len(str(item[layout.index('slides_url')])) > 5:
            md += f"\n<a href='{item[layout.index('slides_url')]}'>Download slides here</a>\n"
        if len(str(item[layout.index('paper_url')])) > 5:
        ...
        # Write the file
        md_filename = os.path.join(PUBLICATIONS_DIR, os.path.basename(md_filename))
